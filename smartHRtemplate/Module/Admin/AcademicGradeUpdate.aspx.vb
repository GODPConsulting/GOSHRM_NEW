...
Public Class AcademicGradeUpdate
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            'Process.LoadRadCombo(radEducationLevel, "Recruit_Academic_Grade_Level_Get_all", 0)
            If Not Me.IsPostBack Then
                'Company_Structure_get_parent
                Session("clicked") = "2"
                If Request.QueryString("id") IsNot Nothing Then
                    Dim strUser As New DataSet
                    strUser = SqlHelper.ExecuteDataset(WebConfig.ConnectionString, "Recruit_Academic_Grade_get", Request.QueryString("id"))
                    txtid.Text = strUser.Tables(0).Rows(0).Item("id").ToString
                    aname.Value = strUser.Tables(0).Rows(0).Item("name").ToString
                    arank.Value = strUser.Tables(0).Rows(0).Item("level").ToString
                    adesc.Value = strUser.Tables(0).Rows(0).Item("Desc").ToString
                Else
                    txtid.Text = "0"
                End If
    ...
    Protected Sub btnAdd_Click(sender As Object, e As EventArgs)
        Try
            ...
            If txtid.Text = "0" Then
                ...
                End If
            Else
                SqlHelper.ExecuteNonQuery(WebConfig.ConnectionString, "Recruit_Academic_Grade_Update", txtid.Text, aname.Value.Trim, adesc.Value, arank.Value)
            End If
    ...
    Private Function GetIdentity() As String
        Try
            ...
            cmd.CommandText = "Recruit_Academic_Grade_Update"
            cmd.Parameters.Add("@id", SqlDbType.VarChar).Value = txtid.Text
            cmd.Parameters.Add("@name", SqlDbType.VarChar).Value = aname.Value.Trim
            cmd.Parameters.Add("@desc", SqlDbType.VarChar).Value = adesc.Value.Trim
