...
Public Class WorkWeekUpdate
        ...
        Try
            If Not Me.IsPostBack Then
                ...
                If Request.QueryString("id") IsNot Nothing Then
                    ...
                    strUser = SqlHelper.ExecuteDataset(WebConfig.ConnectionString, "Work_Week_get", Request.QueryString("id"))
    ...
    Protected Sub btnAdd_Click(sender As Object, e As EventArgs)
        Try
            ...
            If Request.QueryString("id") IsNot Nothing Then
                ...
                olddata(1) = strUser.Tables(0).Rows(0).Item("Day").ToString
                olddata(2) = strUser.Tables(0).Rows(0).Item("Status").ToString
                olddata(3) = strUser.Tables(0).Rows(0).Item("country").ToString
            End If

            If txtid.Text.Trim = "" Then
                workweek.id = 0
            Else
                workweek.id = txtid.Text
            End If
            workweek.Day = radDay.SelectedText
            workweek.Country = radCountry.SelectedText
            workweek.Status = radStatus.SelectedText
            ...
            If Request.QueryString("id") IsNot Nothing Then 'Updates
                For Each a In GetType(clsWorkWeek).GetProperties()
                    If a.Name.ToLower <> "id" And a.Name.ToLower <> "password" Then
                        If a.PropertyType.IsValueType = True Or a.PropertyType.Equals(GetType(String)) Then
                            If IsNumeric(a.GetValue(workweek, Nothing)) = True And IsNumeric(olddata(j)) = True Then
                                If CDbl(a.GetValue(workweek, Nothing)) <> CDbl(olddata(j)) Then
                                    NewValue += a.Name + ": " + a.GetValue(workweek, Nothing).ToString & vbCrLf
                                    OldValue += a.Name + ": " + olddata(j).ToString & vbCrLf
                                End If
                            Else
                                If a.GetValue(workweek, Nothing).ToString <> olddata(j).ToString Then
                                    NewValue += a.Name + ": " + a.GetValue(workweek, Nothing).ToString & vbCrLf
                                    OldValue += a.Name + ": " + olddata(j).ToString & vbCrLf
                                End If
                            End If
                ...
                Next
            Else
                For Each a In GetType(clsWorkWeek).GetProperties() 'New Entries
                    If a.Name.ToLower <> "id" And a.Name.ToLower <> "password" Then
                        If a.PropertyType.IsValueType = True Or a.PropertyType.Equals(GetType(String)) Then
                            If a.GetValue(workweek, Nothing) = Nothing Then
            ...
            If txtid.Text = "0" Then
                ...
                End If
            Else
                SqlHelper.ExecuteNonQuery(WebConfig.ConnectionString, "Work_Week_update", workweek.id, workweek.Day, workweek.Status, workweek.Country)
            End If
            If NewValue.Trim = "" And OldValue.Trim = "" Then
            Else
                If Request.QueryString("id") IsNot Nothing Then
                    Dim saveAudit As Boolean = Process.GetAuditTrailInsertandUpdate(OldValue, NewValue, "Updated", "Work Week")
                Else
                    Dim saveAudit As Boolean = Process.GetAuditTrailInsertandUpdate(OldValue, NewValue, "Inserted", "Work Week")
                End If
    ...
    Private Function GetIdentity() As String
        Try
            ...
            cmd.CommandText = "Work_Week_update"
    ...
    Protected Sub btnCancel_Click(sender As Object, e As EventArgs)
        Try
            Response.Redirect("~/Module/Admin/WorkWeek", True)
        Catch ex As Exception
            Process.loadalert(divalert, msgalert, ex.Message, "danger")
        End Try
    End Sub
